defmodule GraphBLAS.Backend.SuiteSparse do
  ...
  alias GraphBLAS.{Error, Matrix, Monoid, Scalar, Semiring, Vector}
  ...
  @impl GraphBLAS.Backend
  def matrix_mxv(%Matrix{data: %{ptr: m_ptr}}, %Vector{data: %{ptr: v_ptr}}, semiring, _opts) do
    with {:ok, sr} <- resolve_semiring(semiring) do
      semiring_code = semiring_to_code(sr)

      case GraphBLAS.Native.matrix_mxv(m_ptr, v_ptr, semiring_code) do
        ptr when is_integer(ptr) ->
  ...
  def matrix_ewise_add(
    ...
    with {:ok, m} <- resolve_monoid(monoid) do
      ...
      case GraphBLAS.Native.matrix_ewise_add(a_ptr, b_ptr, monoid_code) do
        ptr when is_integer(ptr) ->
  ...
  def matrix_ewise_mult(
    ...
    with {:ok, m} <- resolve_monoid(monoid) do
      ...
      case GraphBLAS.Native.matrix_ewise_mult(a_ptr, b_ptr, monoid_code) do
        ptr when is_integer(ptr) ->
          {:ok, %Matrix{shape: a.shape, type: type, data: %{ptr: ptr}}}
  ...
  @impl GraphBLAS.Backend
  def matrix_reduce(%Matrix{data: %{ptr: ptr}, type: type}, monoid, _opts) do
    with {:ok, m} <- resolve_monoid(monoid) do
      monoid_code = monoid_to_code(m)

      case GraphBLAS.Native.matrix_reduce_to_vector(ptr, monoid_code) do
  ...
  @impl GraphBLAS.Backend
  def matrix_transpose(%Matrix{data: %{ptr: ptr}, type: type}, _opts) do
    case GraphBLAS.Native.matrix_transpose(ptr) do
  ...
  @impl GraphBLAS.Backend
  def vector_vxm(%Vector{data: %{ptr: v_ptr}}, %Matrix{data: %{ptr: m_ptr}}, semiring, _opts) do
    with {:ok, sr} <- resolve_semiring(semiring) do
      semiring_code = semiring_to_code(sr)

      case GraphBLAS.Native.vector_vxm(v_ptr, m_ptr, semiring_code) do
        ptr when is_integer(ptr) ->
          size = GraphBLAS.Native.vector_size(ptr)
          {:ok, %Vector{size: size, type: sr.type, data: %{ptr: ptr}}}
  ...
  def vector_ewise_add(
    ...
    with {:ok, m} <- resolve_monoid(monoid) do
      ...
      case GraphBLAS.Native.vector_ewise_add(a_ptr, b_ptr, monoid_code) do
        ptr when is_integer(ptr) ->
          {:ok, %Vector{size: size, type: type, data: %{ptr: ptr}}}
  ...
  def vector_ewise_mult(
    ...
    with {:ok, m} <- resolve_monoid(monoid) do
      ...
      case GraphBLAS.Native.vector_ewise_mult(a_ptr, b_ptr, monoid_code) do
        ptr when is_integer(ptr) ->
          {:ok, %Vector{size: size, type: type, data: %{ptr: ptr}}}
